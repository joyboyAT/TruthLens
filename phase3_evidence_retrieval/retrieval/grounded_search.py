...
class GroundedSearcher:
	...
	def search(self, claim_text: str, entities: Optional[List[str]] = None, top_k: int = 5, days: Optional[int] = 90) -> List[RawEvidence]:
		query = self.build_query(claim_text, entities, days)
		results = self.client.search(query, num_results=top_k, days=days)
		normalized: List[RawEvidence] = []
		for r in results:
			if not r.url:
				continue
			try:
				...
				evidence = RawEvidence(
					...
					text_hash=None,
					metadata=metadata,
					source_config={"query": query, "client": self.client.__class__.__name__},
					retrieval_metadata={"score": r.score},
				)
