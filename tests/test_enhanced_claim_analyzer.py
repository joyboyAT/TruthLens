#!/usr/bin/env python3
"""
Enhanced Claim Analyzer Test
Tests the enhanced claim analyzer with phrase-based search and News API best practices.
"""
...
def print_analysis_result(result: ClaimAnalysisResult):
    ...
    print(f"\n{'='*80}")
    print(f"ENHANCED CLAIM ANALYSIS RESULTS")
    print(f"{'='*80}")
...
def get_test_claims() -> List[str]:
    """Get test claims for analysis with different types."""
    return [
        # Causal claims
...
def main():
    """Main function to test the enhanced claim analyzer."""
    try:
        # API keys
        ...
        print("🚀 Initializing Enhanced Claim Analyzer...")
        ...
        print(f"\n🔍 Testing {len(test_claims)} claims with enhanced analysis...")
        print("="*80)
        print("⚠️  Using conservative rate limiting to avoid 429 errors")
        print("⏱️  Delays: 500ms between API calls, 5s between claims")
        print("="*80)
        ...
        output_file = "enhanced_claim_analysis_results.json"
        ...
        if successful_analyses == len(test_claims):
            print("\n✅ All enhanced claim analyses completed successfully!")
            return 0
        else:
